...
public class ExpandableTextView extends TextView {
	...
	public boolean mExpanded = DEFAULT_EXPANDED;
	...
	private int textActuralLines = 0;

	// region Constructor

	public int getTextActuralLines() {
		return textActuralLines;
	}

	public void setTextActuralLines(int textActuralLines) {
		this.textActuralLines = textActuralLines;
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mExpanded = typedArray.getBoolean(
				R.styleable.ExpandableTextView_expanded, DEFAULT_EXPANDED);
		mMinLines = typedArray.getInteger(
				R.styleable.ExpandableTextView_minLines, DEFAULT_MIN_LINES);
		typedArray.recycle();
	}
	...
	public interface OnLineListener {
		public void OnLineGet(int lines);
	}

	@Override
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		LogUtil.d("height:%s", heightMeasureSpec);
		if (!mAnimating) {
			heightMeasureSpec = 0;
		}
		...
		if (!mInitialized) {
			LogUtil.d("%s"
					, (getText().toString().length() > 50 ? getText()
							.toString().substring(0, 20) : getText().toString()
							.trim()));
			// mOriginalWidth = getMeasuredWidth();
			// mOriginalHeight = getMeasuredHeight();
			// textActuralLines = getLineCount();
			// if (listener != null)
			// listener.OnLineGet(textActuralLines);
			// setMaxLines(mMinLines);
			// super.onMeasure(widthMeasureSpec, heightMeasureSpec);
			// mCollapseHeight = getMeasuredHeight();
			// mInitialized = true;
			// setMeasuredDimension(mOriginalWidth, mExpanded ? mOriginalHeight
			// : mCollapseHeight);

			mOriginalWidth = getMeasuredWidth();
			// mOriginalHeight = getMeasuredHeight();
			mOriginalHeight = getLayout().getLineTop(getLineCount());
			textActuralLines = getLineCount();
			if (listener != null)
				listener.OnLineGet(textActuralLines);
			setMaxLines(mMinLines);
			super.onMeasure(widthMeasureSpec, heightMeasureSpec);
			mCollapseHeight = getLayout().getLineTop(mMinLines);
			mInitialized = true;
			setMeasuredDimension(mOriginalWidth, mExpanded ? mOriginalHeight
					: mCollapseHeight);

			LogUtil.d("%s" ,mExpanded + "," + mOriginalHeight + ","
					+ mCollapseHeight);

		} else if (getTag(R.id.tag_expandable_text_view_reused) != null
				&& !mAnimating) {
	...
	public ExpandableTextView setOnExpandListener(
			OnExpandListener onExpandListener) {
		mOnExpandListener = onExpandListener;
		return this;
	}

	public ExpandableTextView setOnCollapseListener(
	...
	/**
	 * If you use animation and the animation is not finished, it will return
	 * the previous state.
	 */
	...
	/**
	 * When you use this method in ListView with ViewHolder pattern, set it when
	...
	private class ExpandAnimation extends Animation {
		...
		protected void applyTransformation(float interpolatedTime,
				Transformation t) {
			changeExpanderHeight(mStartHeight
	...
	private class ExpandAnimationListener implements
